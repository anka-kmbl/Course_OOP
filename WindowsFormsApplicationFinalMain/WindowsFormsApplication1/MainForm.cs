    ...
    public partial class MainForm : Form
    {
        ...
        private void AddMBox_Click(object sender, EventArgs e)
        {
            ...
            else if (Convert.ToInt32(SumInput.Text) >= 10 && Convert.ToInt32(SumInput.Text) <= 500)
            {
                ...
                Cheque.Add("Додано скарбничку, сума вкладу: " + SumInput.Text);
                SumInput.Clear();
                ShowInfo();
            }
        ...
        private void BalanceCheck_Click(object sender, EventArgs e)
        {
                ...
                for (int i = 0; i < CurrentClient.Holdings.Count; i++)
                {
                    if (CurrentClient.Holdings[i].DepositName == 1)
                    {
                        if (CurrentClient.Holdings[i].CalcNumMonthLeft(DateTime.Now) <= 0)
                        {
                            ...
                            Cheque.Add("Термін депозита вичерпано. Суму у розмірі: " + CurrentClient.Holdings[i].CalcFinalSum(DateTime.Now) + " повернуто.");
            ...
            {
                MessageBox.Show("У вас ще немає депозитів та скарбничок." + Environment.NewLine + "Ви можете створити їх за допомогою відповідних кнопок.");
            }
        }
            ...
            for(int i = 0; i < CurrentClient.Holdings.Count; i++)
            {
                if(CurrentClient.Holdings[i].DepositName == 1)
                {
                    if (StopDepNum.Value == count)
                    {
                        ...
                        Cheque.Add("Розірвання контракту за депозитом " + count + Environment.NewLine + "Суму у розмірі: " + CurrentClient.Holdings[i].FinalAmount + " повернено.");
                        CurrentClient.Holdings.Remove(CurrentClient.Holdings[i]);
                        i--;
                    }
                    count++;
            ...
            for (int i = 0; i < CurrentClient.Holdings.Count; i++)
            {
                if (CurrentClient.Holdings[i].DepositName == 2)
                {
                    if (WithdrawMBoxNum.Value == count)
                    {
                            ...
                            MessageBox.Show("Ви зняли всю суму зі скарбнички. Її буде видалено.");
                        Cheque.Add("Зняття повної суми у розмірі " + CurrentClient.Holdings[i].CalcSum(DateTime.Now) + " зі скарбнички номер " + count + Environment.NewLine + "Скарбничку номер " + count + " видалено.");
                        CurrentClient.Holdings.Remove(CurrentClient.Holdings[i]);
                            i--;
